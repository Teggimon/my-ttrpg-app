...
| Modal appear | Slide up from bottom (mobile) — `translateY(100%) → translateY(0)`, 250ms ease-out |
...
All secondary screens are bottom-sheet modals (slide up from bottom) unless otherwise noted. A `Cancel` button sits in the footer of each. A dedicated ✕ close button in the top-right corner of each modal is **to be added in a later pass** — the Cancel button covers the basic case for now.
